    ...
    <div className="space-y-5 max-w-5xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-surface-400 text-sm mt-0.5">
            <span className="font-mono text-brand-400">{users.length}</span>{" "}
            pengguna terdaftar
          </p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-surface-700 bg-surface-900/50">
                {["Pengguna", "Role", "Status", "Bergabung", "Aksi"].map(
                  (h) => (
                    <th
                      key={h}
                      className="text-left py-3 px-4 text-xs font-medium text-surface-400 uppercase tracking-wider"
                    >
                      {h}
                    </th>
                  ),
            ...
            <tbody className="divide-y divide-surface-800/50">
              {users.map((u) => {
                ...
                return (
                  <tr
                    key={u.id}
                    className={`hover:bg-surface-800/30 transition-colors ${!u.is_active ? "opacity-50" : ""}`}
                  >
                      ...
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                            isCurrentUser
                              ? "bg-brand-700 ring-2 ring-brand-500"
                              : "bg-surface-700"
                          }`}
                        >
                    ...
                    <td className="py-3 px-4">
                      <span
                        className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs border ${roleColors[rName] || roleColors.viewer}`}
                      >
                        <Shield size={10} />
                        {roleLabels[rName] || rName || "—"}
                      </span>
                    </td>
                    <td className="py-3 px-4">
                      {u.is_active ? (
      ...
      {modal && (
        <div className="absolute inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="mt-60 bg-surface-900 border border-surface-700 rounded-2xl p-6 w-full max-w-md shadow-2xl animate-in">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  {roles
                    .filter((r) =>
                      isSuperAdmin ? true : r.name !== "super_admin",
                    )
                    .map((r) => (
