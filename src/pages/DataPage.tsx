...
type Filters = { search: string; dateFrom: string; dateTo: string }
...
export default function DataPage() {
  ...
  type RenderEntry =
      | { kind: 'standalone'; item: InspeksiBarang }
  ...
  // ── House row ──────────────────────────────────────────────────────────────
  const renderHouseRow = (item: InspeksiBarang, groupOriDest: string, groupAirlineCode: string) => {
        ...
        <div key={item.id} onClick={() => navigate(`/data/${item.id}`)}
          ...
          <div className="flex-[2] min-w-[80px] flex items-center">
            <span className="text-sm font-semibold text-gray-600 break-words">
              {airlineCode && oriDest ? oriDest : oriDest || airlineCode || 'CGK-SIN'}
            </span>
          </div>
  ...
  const renderStandaloneRow = (item: InspeksiBarang) => {
    const isChecked   = selected.includes(item.id)
    // ── NEW: look up gudang by the item's blawb (mawb or hawb) ──
  ...
  const renderGroupHeader = (mawb: string, items: InspeksiBarang[], resolvedOriDest: string, resolvedAirlineCode: string) => {
    ...
    return (
        <div className="min-h-[52px] px-4 border-l border-r border-b border-gray-300 bg-blue-50 flex flex-wrap items-center gap-2">
          ...
          <div className="flex-[2] min-w-[80px] flex items-center">
            <span className="text-sm font-semibold text-gray-500 break-words">
              {resolvedAirlineCode && resolvedOriDest ? `${resolvedAirlineCode} / ${resolvedOriDest}` : resolvedOriDest || resolvedAirlineCode || ''}
            </span>
          </div>
          <div className="flex-1 min-w-[50px] flex items-center gap-1">
        ...
        <div className="flex flex-col shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]">
          ...
          <div className="h-16 px-4 bg-slate-100 rounded-tl-lg rounded-tr-lg border border-gray-300 flex items-center gap-2">
            ...
            {[
              ...
              { label: 'ORI - DEST',  field: 'ori_dest',      flex: 'flex-[2] min-w-[80px]'  },
                ...
                <div key={`group-${entry.mawb}-${i}`} className="relative">
                  ...
                  {!isCollapsed && (
                      <div className="border-l-2 border-orange-200">
                        {entry.items.map(item => renderHouseRow(item, entry.resolvedOriDest, entry.resolvedAirlineCode))}
                      </div>
                  )}
                </div>
            )
